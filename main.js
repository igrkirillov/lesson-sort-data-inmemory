...
var _marked = /*#__PURE__*/_regeneratorRuntime().mark(generateSortParams);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  data = getData();
  rendererData(data);
  var sortParamsGenerator = generateSortParams();
  var sortExecFunc = function sortExecFunc() {
    var sortParams = sortParamsGenerator.next().value;
    sortTable(sortParams.column, sortParams.direction);
  };
  intervalId = setInterval(sortExecFunc, 2000);
});
function generateSortParams() {
  var _i, _arr, direction, _i2, _arr2, column;
  return _regeneratorRuntime().wrap(function generateSortParams$(_context) {
    while (1) switch (_context.prev = _context.next) {
      ...
      case 2:
        ...
        direction = _arr[_i];
        _i2 = 0, _arr2 = ["id", "title", "year", "imdb"];
      case 5:
        if (!(_i2 < _arr2.length)) {
          _context.next = 12;
          break;
...
function rendererData(data, sortColumn, directionSort) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var trData = _step.value;
      var trElement = createTrElement(trData);
  ...
  var _iterator2 = _createForOfIteratorHelper(tableElement.querySelectorAll("tr th")),
    _step2;
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      var th = _step2.value;
      var thContent = th.textContent;
      if (thContent === sortColumn) {
        th.classList.add("sort-".concat(directionSort));
      } else {
...
function sortTable(column, direction) {
  var sortedData = data.sort(function (row1, row2) {
...
function clearTable() {
  var trList = Array.from(tableElement.querySelectorAll("tr")).slice(1);
  trList.forEach(function (el) {
    return tableElement.removeChild(el);
  });
}
